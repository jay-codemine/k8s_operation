// Jenkinsfile - Java Maven 项目（containerd + nerdctl + digest 部署）
// =============================================================
// 架构：Jenkins 虚拟机 -> Maven 构建 -> nerdctl 打镜像 -> 推送 Harbor -> 回调平台
...
pipeline {
    ...
    post {
        success {
            echo "=== 构建成功，回调平台触发部署 ==="
            script {
                callbackPlatform('SUCCESS', 'Java 项目镜像构建推送成功')
            }
        }
        failure {
            echo "=== 构建失败，回调平台 ==="
            script {
                callbackPlatform('FAILURE', "Java 项目构建失败: ${currentBuild.result}")
            }
        }
        aborted {
            echo "=== 构建中止，回调平台 ==="
