...
pipeline {
    ...
    environment {
        ...
        BUILD_IMAGE = ''
        IMAGE_DIGEST = ''
        BUILD_ERROR_MESSAGE = ''  // 构建失败时的错误信息
        FAILED_STAGE_NAME = ''    // 失败的阶段名称
            ...
            steps {
                script {
                    try {
                        ...
                        env.BUILD_ERROR_MESSAGE = "代码检出失败: ${e.message}"
                        env.FAILED_STAGE_NAME = '代码检出'
                        throw e
                    }
                }
            }
            ...
            steps {
                script {
                    try {
                        ...
                        env.BUILD_ERROR_MESSAGE = "构建镜像失败: ${e.message}"
                        env.FAILED_STAGE_NAME = '构建'
                        throw e
                    }
                }
            }
            ...
            steps {
                script {
                    try {
                        ...
                        env.BUILD_ERROR_MESSAGE = "推送镜像失败: ${e.message}"
                        env.FAILED_STAGE_NAME = '推送镜像'
                        throw e
                    }
                }
            }
...
def sendCallback(String buildResult) {
    ...
    // 构建错误信息（参考 Jenkins Blue Ocean 风格）
    def errorMessage = ''
    if (status == 'FAILURE') {
        if (env.BUILD_ERROR_MESSAGE) {
            errorMessage = env.BUILD_ERROR_MESSAGE
        } else if (env.FAILED_STAGE_NAME) {
            errorMessage = "阶段 [${env.FAILED_STAGE_NAME}] 执行失败"
        } else {
            errorMessage = '构建失败，请查看构建日志获取详细信息'
        }
    }
