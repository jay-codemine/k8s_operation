...
--      新增: GitOps部署模式(ArgoCD+ArgoWorkflows)支持 + deploy_mode/gitops_config/sync状态字段
--      已有: CICD细粒度权限(21条) + 多级审批 + 审批策略 + test测试环境配置
-- 数据库账号: root / 123456
-- 使用方式（任选其一）:
...
CREATE TABLE IF NOT EXISTS `cicd_pipeline` (
  ...
  -- GitOps 部署模式配置
  `deploy_mode` varchar(20) NOT NULL DEFAULT 'jenkins' COMMENT '部署模式:jenkins,gitops',
  `gitops_config` json DEFAULT NULL COMMENT 'GitOps配置(ArgoCD应用名/仓库/路径/Workflow模板等)',
  -- 部署配置
...
CREATE TABLE IF NOT EXISTS `cicd_pipeline_run` (
  ...
  -- GitOps 跟踪字段 (v2.5.0)
  `workflow_name` varchar(200) NOT NULL DEFAULT '' COMMENT 'Argo Workflow资源名称',
  `argo_app_name` varchar(200) NOT NULL DEFAULT '' COMMENT 'ArgoCD Application名称',
  `sync_revision` varchar(100) NOT NULL DEFAULT '' COMMENT 'ArgoCD同步目标版本(Git SHA)',
  `sync_status` varchar(30) NOT NULL DEFAULT '' COMMENT 'ArgoCD同步状态:Synced/OutOfSync/Unknown',
  PRIMARY KEY (`id`),
...
-- =====================================================
-- GitOps 兼容存量数据库补丁 (v2.5.0)
-- 说明: 全新库 CREATE TABLE 已包含这些字段，存量库通过幂等补丁自动新增
-- =====================================================

-- cicd_pipeline: deploy_mode 字段补丁
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline' AND column_name = 'deploy_mode');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `cicd_pipeline` ADD COLUMN `deploy_mode` VARCHAR(20) NOT NULL DEFAULT ''jenkins'' COMMENT ''部署模式:jenkins,gitops'' AFTER `language_type`', 'SELECT ''column deploy_mode exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline: gitops_config 字段补丁
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline' AND column_name = 'gitops_config');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `cicd_pipeline` ADD COLUMN `gitops_config` JSON DEFAULT NULL COMMENT ''GitOps配置(ArgoCD应用名/仓库/路径/Workflow模板等)'' AFTER `deploy_mode`', 'SELECT ''column gitops_config exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline: deploy_mode 索引补丁
SET @idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline' AND index_name = 'idx_deploy_mode');
SET @sql := IF(@idx_exists = 0, 'ALTER TABLE `cicd_pipeline` ADD INDEX `idx_deploy_mode` (`deploy_mode`)', 'SELECT ''index idx_deploy_mode exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline_run: workflow_name 字段补丁
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline_run' AND column_name = 'workflow_name');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `cicd_pipeline_run` ADD COLUMN `workflow_name` VARCHAR(200) NOT NULL DEFAULT '''' COMMENT ''Argo Workflow资源名称'' AFTER `callback_received`', 'SELECT ''column workflow_name exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline_run: argo_app_name 字段补丁
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline_run' AND column_name = 'argo_app_name');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `cicd_pipeline_run` ADD COLUMN `argo_app_name` VARCHAR(200) NOT NULL DEFAULT '''' COMMENT ''ArgoCD Application名称'' AFTER `workflow_name`', 'SELECT ''column argo_app_name exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline_run: sync_revision 字段补丁
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline_run' AND column_name = 'sync_revision');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `cicd_pipeline_run` ADD COLUMN `sync_revision` VARCHAR(100) NOT NULL DEFAULT '''' COMMENT ''ArgoCD同步目标版本(Git SHA)'' AFTER `argo_app_name`', 'SELECT ''column sync_revision exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline_run: sync_status 字段补丁
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline_run' AND column_name = 'sync_status');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `cicd_pipeline_run` ADD COLUMN `sync_status` VARCHAR(30) NOT NULL DEFAULT '''' COMMENT ''ArgoCD同步状态:Synced/OutOfSync/Unknown'' AFTER `sync_revision`', 'SELECT ''column sync_status exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- cicd_pipeline_run: sync_status 索引补丁
SET @idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'cicd_pipeline_run' AND index_name = 'idx_run_sync_status');
SET @sql := IF(@idx_exists = 0, 'ALTER TABLE `cicd_pipeline_run` ADD INDEX `idx_run_sync_status` (`sync_status`)', 'SELECT ''index idx_run_sync_status exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- =====================================================
-- 完成（仅输出关键信息）
-- =====================================================
SELECT CONCAT('✅ 初始化完成 | 表数: ', COUNT(*), ' | 账号: admin / admin123 | 流水线: ',
              (SELECT COUNT(*) FROM cicd_pipeline), ' | GitOps: v2.5.0') AS result
