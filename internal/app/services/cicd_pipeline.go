...
func (s *Services) PipelineCallback(ctx context.Context, req *requests.PipelineCallbackRequest) (*PipelineCallbackResult, error) {
	...
	if err := s.UpdateBuildStagesComplete(ctx, run.ID, runStatus, image, req.ImageDigest); err != nil {
...
func (s *Services) autoDeployToK8sWithResult(ctx context.Context, pipeline *models.CicdPipeline, image, imageDigest string) *PipelineCallbackResult {
	...
	switch workloadKind {
	case "Deployment":
		_, err = deployment.PatchDeploymentImage(ctx, kubeClient, pipeline.TargetNamespace, pipeline.TargetWorkloadName, pipeline.TargetContainer, finalImage)
	case "StatefulSet":
		err = s.patchStatefulSetImage(ctx, kubeClient, pipeline.TargetNamespace, pipeline.TargetWorkloadName, pipeline.TargetContainer, finalImage)
	case "DaemonSet":
		err = s.patchDaemonSetImage(ctx, kubeClient, pipeline.TargetNamespace, pipeline.TargetWorkloadName, pipeline.TargetContainer, finalImage)
	default:
		err = fmt.Errorf("不支持的工作负载类型: %s", workloadKind)
	}

	if err != nil {
		global.Logger.Error("[自动部署] 工作负载更新失败",
			zap.Int64("pipeline_id", pipeline.ID),
			zap.Error(err),
		)
	...
	_ = s.dao.PipelineUpdateDeployInfo(ctx, pipeline.ID, finalImage, imageDigest, now, "success")

	global.Logger.Info("[自动部署] 工作负载更新成功",
		zap.Int64("pipeline_id", pipeline.ID),
		zap.String("namespace", pipeline.TargetNamespace),
		zap.String("workload", pipeline.TargetWorkloadName),
		zap.String("image", finalImage),
	...
	result.DeployMessage = fmt.Sprintf("部署成功: %s/%s 镜像已更新", pipeline.TargetNamespace, pipeline.TargetWorkloadName)
	return result
}
