...
type NotifyTemplateListResp struct {
	...
	Items []NotifyTemplate `json:"items"`
}
...
func (s *MonitorCRUDService) ListNotifyTemplates(ctx context.Context, req NotifyTemplateListReq) (*NotifyTemplateListResp, error) {
	...
	var total int64
	db.Model(&NotifyTemplate{}).Count(&total)
...
func (s *MonitorCRUDService) GetNotifyTemplate(ctx context.Context, id int64) (*NotifyTemplate, error) {
...
func (s *MonitorCRUDService) CreateNotifyTemplate(ctx context.Context, tpl *NotifyTemplate) error {
	return s.db.WithContext(ctx).Create(tpl).Error
}

// UpdateNotifyTemplate 更新
func (s *MonitorCRUDService) UpdateNotifyTemplate(ctx context.Context, tpl *NotifyTemplate) error {
...
func (s *MonitorCRUDService) DeleteNotifyTemplate(ctx context.Context, id int64) error {
	return s.db.WithContext(ctx).Model(&NotifyTemplate{}).
		Where("id = ? AND is_del = 0", id).
		Update("is_del", 1).Error
}

// PreviewNotifyTemplate 预览模板渲染效果
func (s *MonitorCRUDService) PreviewNotifyTemplate(ctx context.Context, tpl *NotifyTemplate) string {
	// 用示例数据替换变量
	content := tpl.Content
