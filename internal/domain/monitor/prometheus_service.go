...
// NewPrometheusService 创建监控服务
func NewPrometheusService(db *gorm.DB, prometheusURL string, timeoutSec int) *PrometheusService {
	return &PrometheusService{db: db, staticURL: prometheusURL, timeout: QueryTimeout(timeoutSec)}
}
...
// 优先级：ctx.datasource_id（前端指定）> 数据库 is_default=1 > 数据库任一 enabled=1 > config.yaml.staticURL（首次启动兜底）
func (s *PrometheusService) resolveURL(ctx context.Context) string {
	if s.db != nil {
		...
		// 2) 回退：取任一启用的 prometheus/victoriametrics/thanos 数据源（按 ID DESC，最新优先）
	...
	// 3) 兜底：config.yaml 中的 staticURL（首次启动、数据库没任何数据源时使用）
	return s.staticURL
}

// resolveClient 解析 URL 并返回临时 client（每次调用新建）
func (s *PrometheusService) resolveClient(ctx context.Context) (*prom.Client, string, bool) {
	url := s.resolveURL(ctx)
	if url == "" {
		return nil, "", false
	}
	return prom.NewClient(url, s.timeout), url, true
}

// GetPrometheusURL 返回当前使用的 Prometheus 地址（实时解析）
func (s *PrometheusService) GetPrometheusURL(ctx context.Context) string {
	return s.resolveURL(ctx)
}

// IsEnabled 当前是否有可用的 Prometheus 数据源
func (s *PrometheusService) IsEnabled(ctx context.Context) bool {
	return s.resolveURL(ctx) != ""
}

// ===== 辅助方法 =====

func (s *PrometheusService) extractScalar(result *prom.QueryResult) float64 {
	if result == nil {
		return 0
	}
...
func (s *PrometheusService) extractAvgVector(result *prom.QueryResult) float64 {
	if result == nil {
		return 0
	}
	vectors, err := prom.ParseVectorResult(result.Data.Result)
...
func (s *PrometheusService) parseValue(v interface{}) float64 {
	switch val := v.(type) {
	case string:
...
func (s *PrometheusService) GetClusterOverview(ctx context.Context) (*ClusterOverview, error) {
...
func (s *PrometheusService) GetNodeMetrics(ctx context.Context) ([]NodeMetric, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
...
func (s *PrometheusService) GetResourceTrend(ctx context.Context, resource string, duration time.Duration) ([]TrendData, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return nil, fmt.Errorf("Prometheus 未配置")
	}
...
func (s *PrometheusService) GetTopPods(ctx context.Context, metric string, limit int) ([]PodMetric, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return nil, fmt.Errorf("Prometheus 未配置")
	}
...
func (s *PrometheusService) IsHealthy(ctx context.Context) bool {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return false
	}
...
func (s *PrometheusService) GetClusterHealthScore(ctx context.Context) (*HealthScore, error) {
...
// metric: cpu / memory
func (s *PrometheusService) GetNodeHeatmap(ctx context.Context, metric string, duration time.Duration) ([]HeatmapCell, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return nil, fmt.Errorf("Prometheus 未配置")
	}
...
func (s *PrometheusService) GetPodStatusDistribution(ctx context.Context) ([]PodStatusItem, error) {
...
func (s *PrometheusService) GetAbnormalPods(ctx context.Context, minRestarts int, limit int) ([]AbnormalPod, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return nil, fmt.Errorf("Prometheus 未配置")
	}
...
func (s *PrometheusService) GetNamespaceMetrics(ctx context.Context, limit int) ([]NamespaceMetric, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return nil, fmt.Errorf("Prometheus 未配置")
	}
...
func (s *PrometheusService) GetNodeDetail(ctx context.Context, instance string, duration time.Duration) (*NodeDetail, error) {
	client, _, ok := s.resolveClient(ctx)
	if !ok {
		return nil, fmt.Errorf("Prometheus 未配置")
	}
