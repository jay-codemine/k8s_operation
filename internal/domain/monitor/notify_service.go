...
func (s *MonitorCRUDService) ListNotifyChannels(ctx context.Context, req NotifyChannelListReq) (*NotifyChannelListResp, error) {
	...
	var total int64
	db.Model(&NotifyChannel{}).Count(&total)
...
// GetNotifyChannel 详情
func (s *MonitorCRUDService) GetNotifyChannel(ctx context.Context, id int64) (*NotifyChannel, error) {
...
func (s *MonitorCRUDService) CreateNotifyChannel(ctx context.Context, ch *NotifyChannel) error {
	return s.db.WithContext(ctx).Create(ch).Error
}

// UpdateNotifyChannel 更新
func (s *MonitorCRUDService) UpdateNotifyChannel(ctx context.Context, ch *NotifyChannel) error {
...
func (s *MonitorCRUDService) DeleteNotifyChannel(ctx context.Context, id int64) error {
	return s.db.WithContext(ctx).Model(&NotifyChannel{}).
		Where("id = ? AND is_del = 0", id).
		Update("is_del", 1).Error
}

// TestNotifyChannel 测试通知渠道
func (s *MonitorCRUDService) TestNotifyChannel(ctx context.Context, ch *NotifyChannel) (bool, string) {
...
func SendNotification(ch *NotifyChannel, alert *AlertNotification) error {
	switch ch.Type {
	case "dingtalk":
...
func sendDingTalk(ch *NotifyChannel, alert *AlertNotification) error {
	// 自定义模板优先
	if ch.MsgTemplate != "" {
...
func sendFeishu(ch *NotifyChannel, alert *AlertNotification) error {
	if ch.MsgTemplate != "" {
...
func sendWechat(ch *NotifyChannel, alert *AlertNotification) error {
	if ch.MsgTemplate != "" {
		content, err := renderNotifyTemplate(ch.MsgTemplate, alert)
		if err != nil {
			return err
...
func sendWebhook(ch *NotifyChannel, alert *AlertNotification) error {
	if ch.MsgTemplate != "" {
		content, err := renderNotifyTemplate(ch.MsgTemplate, alert)
		if err != nil {
			return err
...
// sendEmail 发送邮件通知
func sendEmail(ch *NotifyChannel, alert *AlertNotification) error {
...
func buildAtConfig(ch *NotifyChannel) map[string]interface{} {
...
func injectFeishuKeyword(ch *NotifyChannel, content string) string {
...
type RoutePolicyListResp struct {
	Total int64                              `json:"total"`
	Items []NotifyRoutePolicy `json:"items"`
}
...
func (s *MonitorCRUDService) ListRoutePolicies(ctx context.Context, req RoutePolicyListReq) (*RoutePolicyListResp, error) {
	...
	var total int64
	db.Model(&NotifyRoutePolicy{}).Count(&total)
...
// GetRoutePolicy 详情
func (s *MonitorCRUDService) GetRoutePolicy(ctx context.Context, id int64) (*NotifyRoutePolicy, error) {
	var policy NotifyRoutePolicy
...
func (s *MonitorCRUDService) CreateRoutePolicy(ctx context.Context, policy *NotifyRoutePolicy) error {
	// 如果设为默认策略，取消其他默认
	if policy.IsDefault {
		s.db.WithContext(ctx).Model(&NotifyRoutePolicy{}).
...
func (s *MonitorCRUDService) UpdateRoutePolicy(ctx context.Context, policy *NotifyRoutePolicy) error {
	// 如果设为默认策略，取消其他默认
	if policy.IsDefault {
		s.db.WithContext(ctx).Model(&NotifyRoutePolicy{}).
...
func (s *MonitorCRUDService) DeleteRoutePolicy(ctx context.Context, id int64) error {
	return s.db.WithContext(ctx).Model(&NotifyRoutePolicy{}).
		Where("id = ? AND is_del = 0", id).
		Update("is_del", 1).Error
}
...
func (s *MonitorCRUDService) ResolveRoutePolicyChannels(ctx context.Context, rule *AlertRule) string {
	var policies []NotifyRoutePolicy
