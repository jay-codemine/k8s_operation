...
func (s *MonitorCRUDService) ImportAlertRulesFromYAML(ctx context.Context, req AlertRuleImportReq) (*AlertRuleImportResult, error) {
	...
	var routePolicies []NotifyRoutePolicy
	if req.AutoRoute {
...
func (s *MonitorCRUDService) matchRoutePolicy(policies []NotifyRoutePolicy, severity, group string, labels map[string]string) string {
...
func (s *MonitorCRUDService) policyMatches(policy *NotifyRoutePolicy, severity, group string, labels map[string]string) bool {
...
type MonitorCRUDService struct {
	db   *gorm.DB          // 保留用于复杂查询（YAML 导入导出等），逐步迁移到 repo
	repo MonitorRepository // 核心 CRUD 仓储
}

func NewMonitorCRUDService(db *gorm.DB, repo MonitorRepository) *MonitorCRUDService {
	return &MonitorCRUDService{db: db, repo: repo}
}
...
func (s *MonitorCRUDService) ListDatasources(ctx context.Context, req DatasourceListReq) (*DatasourceListResp, error) {
	...
	var total int64
	db.Model(&Datasource{}).Count(&total)
...
// GetDatasource 详情
func (s *MonitorCRUDService) GetDatasource(ctx context.Context, id int64) (*Datasource, error) {
	return s.repo.DatasourceFindByID(ctx, id)
}

// CreateDatasource 创建
func (s *MonitorCRUDService) CreateDatasource(ctx context.Context, ds *Datasource) error {
	ds.Status = "unknown"
...
// UpdateDatasource 更新
func (s *MonitorCRUDService) UpdateDatasource(ctx context.Context, ds *Datasource) error {
	db := s.db.WithContext(ctx)
...
func (s *MonitorCRUDService) DeleteDatasource(ctx context.Context, id int64) error {
	return s.repo.DatasourceDelete(ctx, id)
}

// TestDatasourceConnection 测试数据源连通性
func (s *MonitorCRUDService) TestDatasourceConnection(ctx context.Context, ds *Datasource) (bool, string) {
...
func (s *MonitorCRUDService) updateDatasourceStatus(ctx context.Context, id int64, status string) {
	if id > 0 {
		s.db.WithContext(ctx).Model(&Datasource{}).
			Where("id = ?", id).
			Updates(map[string]interface{}{
				"status":        status,
...
func (s *MonitorCRUDService) ListAlertRules(ctx context.Context, req AlertRuleListReq) (*AlertRuleListResp, error) {
	...
	var total int64
	db.Model(&AlertRule{}).Count(&total)
...
// GetAlertRule 详情
func (s *MonitorCRUDService) GetAlertRule(ctx context.Context, id int64) (*AlertRule, error) {
	return s.repo.AlertRuleFindByID(ctx, id)
}

// CreateAlertRule 创建
func (s *MonitorCRUDService) CreateAlertRule(ctx context.Context, rule *AlertRule) error {
	return s.repo.AlertRuleSave(ctx, rule)
}

// UpdateAlertRule 更新
func (s *MonitorCRUDService) UpdateAlertRule(ctx context.Context, rule *AlertRule) error {
...
func (s *MonitorCRUDService) BatchDeleteAlertRules(ctx context.Context, ids []int64) (*BatchDeleteResult, error) {
	...
	tx := s.db.WithContext(ctx).Model(&AlertRule{}).
...
func (s *MonitorCRUDService) resolveRuleIDsByFilter(ctx context.Context, req BatchBindChannelsReq) ([]int64, string, error) {
	...
	db := s.db.WithContext(ctx).Model(&AlertRule{}).Where("is_del = 0 AND enabled = 1")
...
func (s *MonitorCRUDService) executeBatchBind(ctx context.Context, req BatchBindChannelsReq) (*BatchBindChannelsResult, error) {
	...
	case "append":
		...
		for _, ruleID := range req.RuleIDs {
			...
			if err := s.db.WithContext(ctx).Model(&AlertRule{}).
...
func (s *MonitorCRUDService) ListAlertEvents(ctx context.Context, req AlertEventListReq) (*AlertEventListResp, error) {
	db := s.db.WithContext(ctx).Model(&AlertEvent{})
...
// GetAlertEvent 详情
func (s *MonitorCRUDService) GetAlertEvent(ctx context.Context, id int64) (*AlertEvent, error) {
	event, err := s.repo.AlertEventFindByID(ctx, id)
	if err != nil {
		return nil, err
	}
	// 补渲染未解析的模板变量
...
func (s *MonitorCRUDService) AckAlertEvent(ctx context.Context, id int64, userID int64) error {
	return s.db.WithContext(ctx).Model(&AlertEvent{}).
...
func (s *MonitorCRUDService) BatchDeleteAlertEvents(ctx context.Context, ids []int64) (*BatchDeleteResult, error) {
	...
	tx := s.db.WithContext(ctx).Where("id IN ?", ids).Delete(&AlertEvent{})
	if tx.Error != nil {
		return nil, tx.Error
	}
	return &BatchDeleteResult{
...
func (s *MonitorCRUDService) BatchUpdateNotifyChannels(ctx context.Context, req BatchUpdateNotifyChannelsReq) (*BatchUpdateResult, error) {
	...
	tx := s.db.WithContext(ctx).Model(&NotifyChannel{}).
		Where("id IN ?", req.IDs).
		Updates(updates)
	if tx.Error != nil {
		return nil, tx.Error
...
func (s *MonitorCRUDService) BatchDeleteNotifyChannels(ctx context.Context, ids []int64) (*BatchDeleteResult, error) {
	...
	tx := s.db.WithContext(ctx).Where("id IN ?", ids).Delete(&NotifyChannel{})
	if tx.Error != nil {
		return nil, tx.Error
	}
	return &BatchDeleteResult{
...
func (s *MonitorCRUDService) BatchDeleteSilenceRules(ctx context.Context, ids []int64) (*BatchDeleteResult, error) {
	...
	tx1 := s.db.WithContext(ctx).Where("id IN ?", ids).Delete(&SilenceRule{})
	if tx1.Error == nil {
		totalAffected += tx1.RowsAffected
	}
	tx2 := s.db.WithContext(ctx).Where("id IN ?", ids).Delete(&InhibitRule{})
	if tx2.Error == nil {
		totalAffected += tx2.RowsAffected
	}
	tx3 := s.db.WithContext(ctx).Where("id IN ?", ids).Delete(&AggregateRule{})
	if tx3.Error == nil {
...
// ResolvePrometheusURL 解析 Prometheus 数据源地址
// 优先级：DB is_default=1 > DB 任一 enabled=1 > config.yaml 兜底
func (s *MonitorCRUDService) ResolvePrometheusURL(ctx context.Context) string {
	if s.db != nil {
		var ds Datasource
...
// GetDefaultDatasource 获取默认数据源（按类型列表筛选）
func (s *MonitorCRUDService) GetDefaultDatasource(ctx context.Context, types []string) (*Datasource, error) {
	return s.repo.DatasourceFindDefault(ctx, types)
}
