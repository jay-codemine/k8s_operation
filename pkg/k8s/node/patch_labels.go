...
// PatchLabels 修改节点标签（添加/删除）
func PatchLabels(ctx context.Context, kube kubernetes.Interface, nodeName string, addLabels map[string]string, removeLabels []string) error {
	// 构建 JSON Patch
	...
	if err != nil {
		return fmt.Errorf("patch node labels failed: %w", err)
	}
