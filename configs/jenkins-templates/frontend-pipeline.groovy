...
spec:
  containers:
  - name: node
    image: swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/library/node:22-alpine
...
"""
    ...
    parameters {
        ...
        string(name: 'NPM_INSTALL_ARGS', defaultValue: '', description: 'npm 安装额外参数（留空即可；npm ci 失败会自动回退 npm install --legacy-peer-deps。特殊项目可填 --force 等）')
...
FROM swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/nginxinc/nginx-unprivileged:1.27-alpine
