<template>
  <div class="resource-view">
    ...
    <transition name="ddp-slide">
      <div v-if="showDetailModal" class="ddp-overlay" @click.self="showDetailModal = false">
        <div class="ddp-panel">
          <div class="ddp-hd">
            <div class="ddp-hd-body">
              ...
              <div class="ddp-hd-meta" v-if="detailData?.status">
                <span>完成 {{ detailData.status?.succeeded || 0 }}/{{ detailData.spec?.completions || 1 }}</span>
                <span class="ddp-dot">·</span>
                <span>并行 {{ detailData.spec?.parallelism || 1 }}</span>
                <span class="ddp-dot">·</span>
                <span>{{ fmtTime(detailData.metadata?.creationTimestamp) }}</span>
              </div>
            </div>
            <button class="ddp-close-btn" @click="showDetailModal = false"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 6L6 18M6 6l12 12"/></svg></button>
          </div>
          <div v-if="loadingDetail" class="ddp-loading"><div class="ddp-spin"></div><span>正在获取 Job 详情...</span></div>
          <div v-else-if="detailData" class="ddp-body">
            <div class="ddp-sec">
              ...
              <div class="ddp-kv-grid">
                ...
                <div class="ddp-kv"><span class="ddp-k">完成数</span><span class="ddp-v">成功 {{ detailData.status?.succeeded || 0 }} / 目标 {{ detailData.spec?.completions || 1 }}</span></div>
                <div class="ddp-kv"><span class="ddp-k">并行度</span><span class="ddp-v">{{ detailData.spec?.parallelism || 1 }}</span></div>
                <div class="ddp-kv"><span class="ddp-k">退避限制</span><span class="ddp-v">{{ detailData.spec?.backoffLimit ?? 6 }}</span></div>
                <div class="ddp-kv"><span class="ddp-k">活跃 Pod</span><span class="ddp-v">{{ detailData.status?.active || 0 }}</span></div>
                <div class="ddp-kv"><span class="ddp-k">失败次数</span><span class="ddp-v">{{ detailData.status?.failed || 0 }}</span></div>
                <div class="ddp-kv"><span class="ddp-k">开始时间</span><span class="ddp-v">{{ fmtTime(detailData.status?.startTime) }}</span></div>
                <div class="ddp-kv"><span class="ddp-k">完成时间</span><span class="ddp-v">{{ fmtTime(detailData.status?.completionTime) }}</span></div>
              </div>
            </div>
          ...
          <div v-else class="ddp-empty"><div style="font-size:48px;margin-bottom:12px">📭</div><p>暂无 Job 数据</p></div>
        </div>
      </div>
    </transition>
...
const viewJobDetail = async (job) => {
  ...
  try {
    const [detailRes, eventsRes] = await Promise.all([
      jobsApi.detail({ namespace: job.namespace, name: job.name }),
      jobsApi.events({ namespace: job.namespace, name: job.name })
    ])
    detailData.value = detailRes.code === 0 ? (detailRes.data?.data || detailRes.data) : job
...
const ddpStatusText = (d) => {
  ...
  if (d.status?.succeeded >= (d.spec?.completions || 1)) return 'COMPLETE'
  if (d.status?.active > 0) return 'RUNNING'
  if (d.status?.failed > 0) return 'FAILED'
  return 'PENDING'
}
const ddpStatusClass = (d) => {
  const t = ddpStatusText(d)
  if (t === 'COMPLETE') return 'ddp-st-ok'
  if (t === 'RUNNING') return 'ddp-st-prog'
  return 'ddp-st-err'
}
const ddpRefreshEvents = async () => {
  ...
  ddpEventsLoading.value = true
  try {
    const res = await jobsApi.events({ namespace: detailData.value.metadata.namespace, name: detailData.value.metadata.name })
...
.ddp-hd-meta { margin-top: 6px; font-size: 12px; color: rgba(255,255,255,.7); display: flex; gap: 6px; align-items: center; } .ddp-dot { color: rgba(255,255,255,.35); }
...
.ddp-anno-list { padding: 0 20px 12px; } .ddp-anno-row { display: grid; grid-template-columns: 260px 1fr; gap: 8px; padding: 5px 0; border-bottom: 1px solid #f8fafc; font-size: 12px; }
...
.ddp-ctr-row { display: flex; flex-wrap: wrap; align-items: flex-start; gap: 6px; font-size: 12px; } .ddp-rk { flex-shrink: 0; font-weight: 600; color: #475569; min-width: 60px; }
...
.pdp-ev-msg { font-size: 12px; color: #475569; margin-top: 4px; }
